...
| :------ | :------------------------------------------------------------------------------------------------------------------------------------------ | :--------------- | :------------------------------------------------------------------------------------------------------------------------------ | :------- | :---------------------------------------------------------------- |
| P3.1.1  | Create a new user flag `has_completed_onboarding` in the `Profile` table.                                                                   | Backend          | The migration is created and applied. The column defaults to `false`.                                                           | ⬜ To Do |                                                                   |
| P3.1.2  | Create a multi-step wizard component for the onboarding flow.                                                                               | Frontend         | The component manages state for 3 steps: "Loved", "Okay", "Not for You".                                                        | ⬜ To Do | Use a state machine or a simple step counter.                     |
| P3.1.3  | Create a middleware or a check in the root layout to redirect new users (`has_completed_onboarding` is `false`) to the `/onboarding` route. | Both             | **Verification:** A newly signed-up user is forced into the onboarding flow before they can access the main app.                | ⬜ To Do |                                                                   |
| P3.1.4  | Implement the "Loved" step UI, including the TMDB search from Phase 1.                                                                      | Frontend         | The UI prompts the user to find a movie they loved and forces a high rating (e.g., 4.5-5 stars).                                | ⬜ To Do |                                                                   |
| P3.1.5  | Implement the "Okay" and "Not for You" steps.                                                                                               | Frontend         | The UI prompts for movies and constrains the rating to the appropriate range (e.g., 2.5-3.5 for "Okay", 1-2 for "Not for You"). | ⬜ To Do |                                                                   |
| P3.1.6  | Create an `onboardLogEntry` Server Action.                                                                                                  | Backend          | This action takes a `tmdb_id` and a `rating` and creates a `LogEntry` in the user's personal space.                             | ⬜ To Do | This is a simplified version of the main `createLogEntry` action. |
| P3.1.7  | Create a final Server Action `completeOnboarding`.                                                                                          | Backend          | **Verification:** This action sets the `has_completed_onboarding` flag on the user's profile to `true`.                         | ⬜ To Do | This is called after the user submits their third movie.          |
| P3.1.8  | After `completeOnboarding` is successful, redirect the user to their main dashboard.                                                        | Frontend         | The user exits the wizard and lands on their now-populated library page.                                                        | ⬜ To Do |                                                                   |
